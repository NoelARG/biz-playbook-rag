...
export async function checkDocumentStatus(docsDir = "./docs") {
  try {
    const meta = loadMeta();
    ...
    files.forEach(filename => {
      try {
        ...
        // For all file types, we need to read the content to compare with stored checksums
        let content = "";
        if (filename.endsWith('.pdf')) {
          // For PDFs, read the content to calculate a proper checksum
        ...
        // If we have metadata for this file, check if it needs re-processing
        if (existingDocs.has(filename)) {
          const existing = existingDocs.get(filename);
          // Check if we have chunks for this file (means it was processed)
          if (existing.chunks.length > 0) {
            // Check if the file has actually changed by comparing checksums
            const storedChecksum = Array.from(existing.checksums)[0]; // Get first checksum
