...
// Simple, fast document status check
export function checkDocumentStatus(docsDir = "./docs") {
  try {
    const meta = loadMeta();
    const processedSources = new Set(meta.map(m => m.source));
    ...
    // Simple logic: if we have chunks for a file, it's processed
    ...
    processedSources.forEach(filename => {
